...
<a class="card-link" v-on:mouseover="mouseover"
    v-on:mouseleave="mouseleave" :href="recordLink">
  <article :class="blogCardClass">
    ...
    <div class="article-details">
      <h4 class="post-category">Travel {{index}} {{this.message}}</h4>
      <h3 class="post-title">{{record.title}}</h3>
      <p class="post-description">{{record.excerpt}}</p>
      <p class="post-author"><span class="small">{{ record.createdAt }} <strong>&#183;</strong> {{ record.timeToRead }} min read</span></p>
    </div>
    <img v-if="!indexEven" class="post-image" src='https://s3-us-west-2.amazonaws.com/s.cdpn.io/1159990/pike-place.jpg' />
  </article>
</a>
